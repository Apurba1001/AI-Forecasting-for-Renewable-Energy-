...
from config import MODEL_DIR_XGB, OUTPUT_DIR, TARGET_COLS
...
class XGBoostForecaster(BaseForecaster):
    ...
    def _get_prediction_row(
        self, history_df, target_col, real_dt, lookup_dt, country_code, feature_names
    ):
    ...
    def predict(self, country_code: str, forecast_date=None) -> dict:
        ...
        full_df = full_df.drop_duplicates(
            subset=["datetime_utc", "Country"], keep="last"
        )
        ...
        if lookup_start not in country_history.index:
            print(
                f"⚠️ Warning: No historical match found for {ideal_feature_date}. Using last available data."
            )
        ...
        for target in TARGET_COLS:
            ...
            for real_dt, lookup_dt in zip(real_steps, lookup_steps):
                ...
                if (
                    target == "Solar" and real_dt.hour == 12
                ):  # Only check noon (peak solar)
                    ...
                    print(
                        f"   🤖 Model Inputs (First 5 cols): {X_step.iloc[0, :5].to_dict()}"
                    )
        ...
        if forecasts:
            ...
            return {"forecast_data": result_df, "emissions_kg": emissions_kg}
