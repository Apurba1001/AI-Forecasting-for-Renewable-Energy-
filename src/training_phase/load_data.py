...
class EnergyDataLoader:
    ...
    PSR_MAP = {"Solar": "B16", "Wind Onshore": "B19", "Wind Offshore": "B18"}
    ...
    def _fetch_strategy_targeted(
        ...
        for friendly_name, psr_code in self.PSR_MAP.items():
            try:
                part_df = self.client.query_generation(
                    country, start=start, end=end, psr_type=psr_code
                )
    ...
    def run_pipeline(
        self, countries: list, start_date: str, end_date: str, output_path: Path
    ):
