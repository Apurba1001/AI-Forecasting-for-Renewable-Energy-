    ...
    with container:
        ...
        try:
            ...
            carbon_resp = requests.get(
                f"{API_URL}/carbon-live", params=live_params, timeout=1
            )
            ...
            st.markdown(
                '<div class="carbon-live-box" style="background:#374151; color:#9ca3af;">Sensor Unreachable</div>',
                unsafe_allow_html=True,
            )
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if not df.empty:
                ...
                if (
                    "predicted_mw" in df.columns
                    and "Total_Generation" not in df.columns
                ):
                    fig.add_trace(
                        go.Scatter(
                            x=df["datetime_utc"],
